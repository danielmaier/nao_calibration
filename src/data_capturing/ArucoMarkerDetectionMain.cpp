...
int main(int argc, char** argv) {
	ros::init(argc, argv, "arucoMarkerDetectionMain");
...
void imageCb(const sensor_msgs::ImageConstPtr& msg) {
	static ArucoMarkerDetection detection;
	vector<double> data;

	if (!detection.detect(msg, data)) {
		std::cout << "[main] could not detect the aruco marker.\n";
		cmPub.publish(msg);
		return;
	}
...
void detectFromRosMsg() {
	...
	cmPub = it.advertise("/aruco_marker_detection/marker", 1);
	ros::spin();
}
